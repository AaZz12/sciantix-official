...
void GrainBoundaryRupture()
{
    ...
    */
    ...
    double critical_bubble_pressure = equilibriumpressure + 
        1 / factor * sqrt(pi * E * G_gb / ((1.0 - pow(nu, 2)) * sciantix_variable[sv["Integranular bubble radius"]].getFinalValue()));    
    double critical_bubble_pressure_W = equilibriumpressure/2 + 
        sqrt(pow(equilibriumpressure,2)/4 + 
        (3*shearmodulus*G_gb*pi*(pow(spacing,2)-pow(sciantix_variable[sv["Integranular bubble radius"]].getFinalValue(),2))/(4*pow(spacing,3))));    

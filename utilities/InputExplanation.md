...
In this folder are reported the three essential input files required from SCIANTIX
 ...
 - `input_history.txt` containes the time, temperature (K), fission rate (fiss / m3-s)$, and hydrostatic stress (MPa) as a function of time (hr). Steam pressure (atm) can be added if fuel oxidation option is enabled
...
1   #   output -- The output of the simulation is provided in a text file (`output.txt`), in which values are separated by **tabs**. Some output variables changes according to the selected settings.
...
0   #   radioactive fission gases behaviour is considered according to *Zullo et al. (2022) NET*
...
In case of any trouble with those files, please contact the main developers :)

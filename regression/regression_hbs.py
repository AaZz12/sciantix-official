	...
	# Verify on a given folder, if Baker is in it's name
		if "HBS" in file and os.path.isdir(file):
			...
			if mode_plot == 1:
				...
				ax1.scatter(data_bu, data_xe, color='navy', edgecolors='black', marker='.', label='Walker et al. (1999)')
				...
				ax1.plot(sd["bu"] / 0.8814, (sd["xe_igHBS"] + sd["xe_ig"]) / eq, color='dimgray', linestyle='-', label='Xe in grains - total')
				ax1.legend(loc='upper right', fontsize='medium')
				ax1.set_xlabel('Effective burnup (GWd/tU)', fontsize='large')
				ax1.set_ylabel('Xe in grains (wt%)', color='black', fontsize='large')
				ax1.set_ylim(0, 1.75)
				ax2 = ax1.twinx()
				ax2.plot(sd["bu"], sd["alpha"], color='saddlebrown', linestyle='--', label='Restructured volume fraction (/)')

...
class Simulation : public Solver, public Model
{
	...
	void InterGranularBubbleBehaviour()
	{
		...
		std::cout<< "Released fraction: " << sciantix_variable[sv["Intergranular vented fraction"]].getInitialValue() << std::endl;
		std::cout<<"Fractional coverage final value: "<<sciantix_variable[sv["Intergranular fractional coverage"]].getFinalValue()<<std::endl;
		std::cout<< "Released fraction: " << sciantix_variable[sv["Intergranular vented fraction"]].getFinalValue() << std::endl;
		...
		if (sciantix_variable[sv["Intergranular vented fraction"]].getFinalValue())
		{
			...
			for (auto& system : sciantix_system)
			{
				...
				std::cout<<"Fr = "<<sciantix_variable[sv["Intergranular vented fraction"]].getFinalValue()<<std::endl;
				if (system.getRestructuredMatrix() == 0)
				{
					...
					{
						sciantix_variable[sv[system.getGasName() + " released"]].addValue(dR1);
					}
				}
			}
		}
